...
// compound shapes by populating them with a pre-transformed triangle
{
    ...
    if( !arguments.length) return; // Pass no arguments if you just want to make an empty dummy object that inherits everything, for populating other shapes this.populate( this, points_transform ); // Otherwise, a new triangle immediately populates its own arrays with triangle points,
...
triangle.prototype.populate = function( recipient, points_transform ) // The meat of the triangle starts here:
{
    ...
    recipient.normals = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) ];  // norms go toward z
...
function windmill( points_transform )// Argument points_transform: Always identity if we’re just building a windmill. It does good when building other // compound shapes by populating them with a pre-transformed windmill
{
    shape.call(this); // Inherit class shape’s array members by calling parent constructor
...
function lace( points_transform )
{
    shape.call(this);
    if( !arguments.length) return;
    this.populate( this, points_transform );
...
lace.prototype.populate = function( recipient, points_transform )
{
    var offset = recipient.vertices.length;
    var index_offset = recipient.indices.length;
    
    recipient.vertices.push( vec3(-0.5,-0.5,0), vec3(-0.5,0.5,0), vec3(0,0,2) );
    recipient.vertices.push( vec3(-0.5,0.5,0), vec3(0.5,0.5,0), vec3(0,0,2) );
    recipient.vertices.push( vec3(0.5,0.5,0), vec3(0.5,-0.5,0), vec3(0,0,2) );
    recipient.vertices.push( vec3(-0.5,-0.5,0), vec3(0.5,-0.5,0), vec3(0,0,2) );
    ...
    // Base
    recipient.vertices.push( vec3(-0.5,-0.5,0), vec3(-0.5,0.5,0), vec3(0.5,-0.5,0) );
    recipient.vertices.push( vec3(0.5,0.5,0), vec3(-0.5,0.5,0), vec3(0.5,-0.5,0) );
    recipient.texture_coords.push( vec2(0,0), vec2(0,1), vec2(1,0) );
    recipient.texture_coords.push( vec2(0,0), vec2(0,1), vec2(1,0) );
    
    recipient.indices.push( 12, 13, 14, 15, 16, 17 );
    for (var i = 0; i < 2; i++)
...
//function tetrahedron( points_transform ) // Argument points_transform: Always identity if we’re just building a triangle. It does good when building other
//// compound shapes by populating them with a pre-transformed triangle
//{
//    shape.call(this); // Inherit class shape’s array members by calling parent constructor
//    if( !arguments.length) return; // Pass no arguments if you just want to make an empty dummy object that inherits everything, for populating other shapes this.populate( this, points_transform ); // Otherwise, a new triangle immediately populates its own arrays with triangle points,
...
//}
//inherit(tetrahedron, shape);
//tetrahedron.prototype.populate = function( recipient, points_transform ) // The meat of the triangle starts here:
//{
//    var offset = recipient.vertices.length; var index_offset = recipient.indices.length; // Recipient's previous size
//    
//    var a = -1/math.sqrt(3);
...
//    recipient.vertices.push(vec3(0,0,0), vec3(0,1,0), vec3(1,0,0));
//    recipient.vertices.push(vec3(0,0,0), vec3(0,1,0), vec3(0,0,1));
//    recipient.vertices.push(vec3(0,0,0), vec3(1,0,0), vec3(0,0,1));
//    recipient.vertices.push(vec3(0,0,1), vec3(0,1,0), vec3(0,0,1));
//    
//    recipient.normals.push(vec3(0,0,1), vec3(0,0,1), vec3(0,0,1));
//    recipient.normals.push(vec3(1,0,0), vec3(1,0,0), vec3(1,0,0));
//    recipient.normals.push(vec3(0,1,0), vec3(0,1,0), vec3(0,1,0));
//    recipient.normals.push(vec3(a,a,a), vec3(a,a,a), vec3(a,a,a));
//    
////    recipient.vertices.push(vec3(0,0,0), vec3(0,1,0), vec3(1,0,0), vec3(0,0,1));
////    recipient.normals.push(vec3(a,a,a), vec3(0,1,0), vec3(1,0,0), vec3(0,0,1));  // norms go toward z
////    recipient.texture_coords.push(vec2(0,0), vec2(0,1), vec2(1,0), vec2(1,1));
////    recipient.indices.push( 0, 1, 2,    0, 1, 3,    0, 2, 3,    1, 2, 3 );
//    
//    for (var i = index_offset; i < recipient.indices.length; i++)
//        recipient.indices[i] += offset;
//    
